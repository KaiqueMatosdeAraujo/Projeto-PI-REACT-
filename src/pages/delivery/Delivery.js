...
import Cartao from './imgs/cartao-multibanco.png'
...
function Delivery(props) {
  ...
  const getAddress = () => {
    axios.get(`http://localhost:8080/endereco/${idcliente}/1`)
  ...
  const registerOrder = () => {
    axios.post(`http://localhost:8080/pedido/cadastrar`, order)
      .then((response) => {
  ...
  // pedido
  const [montagemPedido, setMontagemPedido] = useState([])

  const [addressCod, setAddressCod] = useState("");

  const addAddress = (event) => {
    setAddressCod(event.target.value);
    setMontagemPedido(addressCod)
    //addOrder(addressCod);
    //localStorage.setItem("order", addressCod);
    //console.log(montagemPedido);
  };
